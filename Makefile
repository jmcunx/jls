...
LFLAGS=$(WALL) $(LIBJLIB2) -o jls $(LIBS)

ALL_OBJ=jls$(OBJ) jls_h$(OBJ) jls_i$(OBJ) jls_u$(OBJ)
ALL_C=jls.c jls_h.c jls_i.c jls_u.c
ALL_H=jls.h

#
# do the work
#
all:	ckenv tags $(ALL_OBJ) jls.1$(GZ)
...
install:	all
	...
	-$(CHMOD) 755 $(PRODMAN)
	-$(STRIP) jls
	$(CP) jls $(PRODUCTION)/jls
	$(CHMOD) 755 $(PRODUCTION)/jls
	$(CP) jls.1$(GZ) $(PRODMAN)/jls.1$(GZ)
	$(CHMOD) 644 $(PRODMAN)/jls.1$(GZ)
